...
let code = fs.readFileSync('src/app/super-admin/exams/edit/[id]/components/QuestionsBuilder.tsx', 'utf8');
...
if (insertIndex > -1) {
  ...
  // also add examData to props extraction
  const propsIndex = lines.findIndex(l => l.includes('const { currentModule, setCurrentModule'));
  if (propsIndex > -1) {
    lines[propsIndex] = lines[propsIndex].replace('const { currentModule,', 'const { examData, currentModule,');
  }

  fs.writeFileSync('src/app/super-admin/exams/edit/[id]/components/QuestionsBuilder.tsx', lines.join('\\n'));
} else {
