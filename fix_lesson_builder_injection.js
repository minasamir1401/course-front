...
hookFiles.forEach(file => {
  if (fs.existsSync(file)) {
    ...
    // First, remove the incorrectly placed moveBlock and insertBlockAt block
    const badRegex = /\s*const moveBlock = \(source: 'slides' \| 'assignments' \| 'questions' = 'slides', index: number, direction: 'UP' \| 'DOWN'\) => \{\s*setCurrentModule\(\(prev: any\) => \{\s*const newSlides = \[\.\.\.\(prev\[source\] \|\| \[\]\)\];\s*const targetIndex = direction === 'UP' \? index - 1 : index \+ 1;\s*if \(targetIndex < 0 \|\| targetIndex >= newSlides\.length\) return prev;\s*\[newSlides\[index\], newSlides\[targetIndex\]\] = \[newSlides\[targetIndex\], newSlides\[index\]\];\s*return \{ \.\.\.prev, \[source\]: newSlides \};\s*\}\);\s*\};\s*const insertBlockAt = \(source: 'slides' \| 'assignments' \| 'questions' = 'slides', index: number, type: 'TEXT' \| 'QUESTION'\) => \{\s*const newBlock = type === 'TEXT'[^]*?return \{ \.\.\.prev, \[source\]: newSlides \};\s*\}\);\s*showToast\("Slide inserted successfully", "success"\);\s*\};\s*/g;
