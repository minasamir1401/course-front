...
import { CheckCircle2, XCircle, ChevronRight, ChevronLeft, LayoutDashboard, RefreshCw, Award, Target, Clock, MessageCircle, Lock, EyeOff, HelpCircle, Info, AlertCircle, Sparkles, BookOpen, MessageSquare, Star, ListOrdered, TrendingUp, Globe } from 'lucide-react';
...
const t = (key: string, lang: 'ar' | 'en') => {
  const translations: Record<string, { ar: string; en: string }> = {
    ...
    passed: { ar: 'تهانينا، لقد نجحت بنجاح!', en: 'Congratulations, you passed!' },
    failed: { ar: 'للأسف، لم يحالفك الحظ هذه المرة', en: "Unfortunately, you didn't pass this time" },
    ...
    passedLabel: { ar: 'ناجح', en: 'Passed' },
    failedLabel: { ar: 'راسب', en: 'Failed' },
...
export default function ExamResultPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] pb-20" dir={dir}>
      ...
      {/* Premium Header */}
      <div className={`relative pt-16 pb-36 px-6 ${isPassed ? "bg-emerald-600" : "bg-rose-600"} text-white text-center overflow-hidden`}>
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <h1 className="text-3xl md:text-5xl font-black mb-3 drop-shadow-sm">
            {isPassed ? t('passed', lang) : t('failed', lang)}
          </h1>
            ...
            <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
              ...
              <h4 className="text-3xl font-black text-slate-800">
                ...
                <span className="text-lg font-bold text-slate-400">
                  {submission.exam.totalPoints || (submission.exam.questions || []).reduce((acc: number, q: any) => acc + parseInt(q.points || 0), 0)}
                </span>
              </h4>
            </div>
            <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-col justify-center">
              <p className="text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest">{t('result', lang)}</p>
              <h4 className={`text-2xl font-black ${isPassed ? "text-emerald-600" : "text-rose-600"}`}>
                {isPassed ? t('passedLabel', lang) : t('failedLabel', lang)}
              </h4>
            </div>
