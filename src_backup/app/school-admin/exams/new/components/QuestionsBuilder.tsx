            ...
            <div className="p-8 md:p-12 space-y-8">
              ...
              <div className="flex justify-between items-center mb-4 mt-6">
                ...
                <div className="flex gap-2">
                  <label className="flex items-center gap-2 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-4 py-2 rounded-xl text-xs font-bold transition-colors border border-indigo-200 cursor-pointer">
                    <Upload className="w-4 h-4" />
                    {language === 'ar' ? 'رفع الميتا داتا' : 'Upload Metadata'}
                    <input 
                      type="file" 
                      accept=".xlsx,.xls" 
                      className="hidden" 
                      onChange={(e) => {
                        ...
                        reader.onload = (evt) => {
                          ...
                          if (data && data.length > 0) {
                            ...
                            let updatedQuestion = { ...tempQuestion };
                            metadataMap.forEach(field => {
                              const val = row[field.labelEn] !== undefined ? row[field.labelEn] : row[field.labelAr];
                              if (val !== undefined) {
                                updatedQuestion[field.key] = val;
                              }
                            });
                  ...
                  >
                    <Download className="w-4 h-4" />
                    {language === 'ar' ? 'تحميل قالب الميتا داتا' : 'Download Metadata'}
                  </button>
                </div>
              </div>
