      ...
      <div className="space-y-10" dir={language === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6 mb-10">
          ...
          <StatCard
            ...
            value={isLoading ? "..." : (stats?.teachersCount || 0)}
            icon={<Users />}
            color="amber"
          ...
          <StatCard
            ...
            value={isLoading ? "..." : (stats?.activeExams || 0)}
            icon={<ClipboardList />}
            color="emerald"
          ...
          <StatCard
            ...
            value={isLoading ? "..." : (stats?.centralExamsCount || 0)}
            icon={<Shield />}
            color="indigo"
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={schoolPerformanceData}>
                    ...
                    <Bar dataKey="students" radius={[8, 8, 0, 0]}>
                      {schoolPerformanceData.map((entry: any, index: number) => (
                ...
                <tbody className="divide-y divide-slate-50">
                  {(stats?.recentSchools || []).map((school: any, i: number) => (
                    <tr key={i} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-8 py-5 text-slate-500 text-sm font-medium">{school.type || 'N/A'}</td>
                      <td className="px-8 py-5">
                      ...
                      <td className="px-8 py-5">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${school.status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                          {school.status === 'ACTIVE' ? t('superAdmin.active') : t('superAdmin.suspended')}
                        </span>
                      </td>
                      <td className="px-8 py-5">
                        <Link
