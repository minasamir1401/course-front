...
function UsersManagementContent() {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedUsers(filteredUsers.filter((u: any) => u.role !== 'SUPER_ADMIN').map((u: any) => u.id));
    } else {
      setSelectedUsers([]);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmed = await confirm(
      ...
      language === 'ar' ? `هل أنت متأكد من حذف ${selectedUsers.length} مستخدم؟` : `Are you sure you want to delete ${selectedUsers.length} users?`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setSelectedUsers([]);
      showToast(language === 'ar' ? 'تم حذف المستخدمين بنجاح' : 'Users deleted successfully', 'success');
    } catch (error) {
      showToast(t('usersPage.connError') || 'Connection error', 'error');
    }
  };
        ...
        <div className="relative overflow-hidden rounded-[32px] bg-white border border-slate-100 shadow-sm p-8 sm:p-12">
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              {selectedUsers.length > 0 && adminRole === 'SUPER_ADMIN' && (
                <button
                  onClick={handleBulkDelete}
                  className="bg-rose-500 hover:bg-rose-600 text-white px-6 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-xl shadow-rose-500/20 hover:scale-105 active:scale-95 text-sm sm:text-base cursor-pointer"
                >
                  <Trash2 className="w-5 h-5" />
                  {language === 'ar' ? `حذف المحددين (${selectedUsers.length})` : `Delete Selected (${selectedUsers.length})`}
                </button>
              )}
              <button
                onClick={() => setIsModalOpen(true)}
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-xl shadow-indigo-600/10 hover:scale-105 active:scale-95 text-sm sm:text-base cursor-pointer w-full sm:w-auto"
              >
                    ...
                    <tr className="text-slate-400 text-xs font-bold uppercase tracking-wider bg-slate-50/50 border-b border-slate-100">
                      <th className="px-6 py-5 w-10 text-center">
                        <input 
                          ...
                          checked={filteredUsers.filter((u: any) => u.role !== 'SUPER_ADMIN').length > 0 && selectedUsers.length === filteredUsers.filter((u: any) => u.role !== 'SUPER_ADMIN').length}
