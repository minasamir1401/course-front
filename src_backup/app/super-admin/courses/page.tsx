    ...
    try {
      ...
      if (reset) {
        ...
        setCourses(prev => {
          ...
          const filteredNew = newCourses.filter((c: any) => !existingIds.has(c.id));
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 sm:space-y-10" dir={language === 'ar' ? 'rtl' : 'ltr'}>
          ...
          {/* ☁️ Subtle cloud sync indicator — shows only during background 3s refresh (Removed as per user request) */}
