      ...
      const timer = setTimeout(() => {
        if (mathContainerRef.current) {
          import('mathlive').then(({ MathfieldElement }) => {
            ...
            // Extract existing LaTeX from value if it contains \(...\)
            // Use a more robust extraction that handles nested braces
            let initialFormula = "";
            const openDelim = "\\(";
            const closeDelim = "\\)";
            const startIdx = value.indexOf(openDelim);
            if (startIdx !== -1) {
              const endIdx = value.lastIndexOf(closeDelim);
              if (endIdx > startIdx) {
                initialFormula = value.substring(startIdx + openDelim.length, endIdx);
              }
            }
