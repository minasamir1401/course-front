...
const flexColRegex = /<div\s+className={`max-w-7xl[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<div className="flex flex-col gap-8">/;
const flexColMatch = content.match(flexColRegex);
if (!flexColMatch) {
  console.log('Flex col start not found');
  process.exit(1);
}
const flexColStart = flexColMatch.index + flexColMatch[0].length;
const beforeFlexCol = content.substring(0, flexColMatch.index);
...
const settingsContentEnd = settingsContentEndMatch > -1 
  ? settingsContentEndMatch 
  : content.indexOf('</div>\r\n                </div>\r\n              </div>\r\n            </div>\r\n          )}', settingsContentStart);
