...
import { CheckCircle2, XCircle, ChevronRight, LayoutDashboard, RefreshCw, Award, Target, Clock, User, Mail, ArrowRight, FileText, BarChart3, HelpCircle, Layers, Building2, TrendingUp, Users } from 'lucide-react';
      ...
      <div className="flex flex-col gap-8 rtl pb-20" dir="rtl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-black text-slate-400">نسبة النجاح العامة</p>
              <p className="text-3xl font-black text-slate-800">{Math.round(data.overall.passRate)}%</p>
            </div>
          </div>
              ...
              <tbody>
                {data.students.map((student: any, i: number) => (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4">
                      <Link 
                        href={\`/\${role === 'super_admin' ? 'super-admin' : 'school-admin'}/exams/submissions/\${student.id}\`}
...
const superAdminPath = 'src/app/super-admin/exams/analytics/[id]';
const schoolAdminPath = 'src/app/school-admin/exams/analytics/[id]';

ensureDirSync(superAdminPath);
ensureDirSync(schoolAdminPath);

fs.writeFileSync(path.join(superAdminPath, 'page.tsx'), pageContent('super_admin'));
fs.writeFileSync(path.join(schoolAdminPath, 'page.tsx'), pageContent('school_admin'));

console.log('Done generating frontend analytics pages');
