...
function reshapeSafe(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');

  const actualStart = content.indexOf('<DashboardLayout hideSidebar>', content.indexOf('<DashboardLayout hideSidebar>') + 100);
...
`;
  ...
  const closingGridDiv = content.lastIndexOf('</div>', dashboardEnd);
...
`;
  ...
  content = content.substring(0, closingGridDiv) + sideBarHtml + content.substring(closingGridDiv);
...
reshapeSafe('src/app/super-admin/exams/edit/[id]/page.tsx');
reshapeSafe('src/app/school-admin/exams/edit/[id]/page.tsx');
