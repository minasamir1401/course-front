...
export const CourseEditorProvider: React.FC<{
  ...
  // Saves lesson data to localStorage every 5s as a safety net
  // =============================================
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        void offlineSync.saveDraft(DRAFT_KEY, draft).catch((error) => {
          console.warn("[CourseEditor] Failed to save IndexedDB draft", error);
        });
        localStorage.setItem(DRAFT_KEY, JSON.stringify(draft));
      } catch (e) {
        console.warn("[CourseEditor] Failed to save local draft", e);
    ...
    try {
      ...
      if (isLessonModalOpen && currentLesson.title) {
        if (editingLessonIndex !== null && editingLessonIndex < lessonsToSend.length) {
          ...
          // New lesson: only push if not already present by title (prevents double-add after saveLesson)
