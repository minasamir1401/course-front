...
export default function RichTextEditor({ value, onChange, placeholder, className = "" }: RichTextEditorProps) {
  ...
  const [activeModal, setActiveModal] = useState<'table' | 'math' | null>(null);
    ...
    input.onchange = (e: any) => {
      ...
      if (file) {
        ...
        reader.onload = (readerEvent) => {
          ...
          const size = window.prompt("اختر عرض الصورة (مثال: 100%, 300px, 500px)", "100%");
          if (size) {
            const imgHtml = `<img src="${content}" style="width: ${size}; max-width: 100%; height: auto; border-radius: 12px; margin: 10px auto; display: block;" />&nbsp;`;
  ...
  const ToolButton = ({ 
    ...
    <button
      ...
      className={`p-2 rounded-xl transition-all duration-200 flex items-center justify-center hover:scale-110 active:scale-95 ${
        active 
  ...
  return (
    <div className={`flex flex-col border-2 rounded-[30px] transition-all duration-300 bg-white ${
      isFocused 
