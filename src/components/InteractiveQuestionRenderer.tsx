    ...
    <div className={`space-y-6 w-full max-w-full ${language === 'ar' ? 'text-right' : 'text-left'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 justify-center">
        {cards.map((c) => {
          ...
          const colorClass = CARD_COLORS[c.pairIndex % CARD_COLORS.length];
          return (
            <button
              key={c.id}
              type="button"
            ...
            >
              {isOpen ? <HtmlRenderer html={translateText(c.text, language)} tag="span" /> : "🌟"}
            </button>
          );
        })}
      </div>
...
function SentenceReorderRenderer({ question, value, onChange, language }: any) {
  ...
  useEffect(() => {
    ...
    if (value && typeof value === 'string') {
      const wordsInValue = value.split(" ").filter(w => w.trim() !== "");
