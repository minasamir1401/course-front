...
const createEmptyItem = (type: ContentType, language?: string): ContentItem => ({
  text: "",
...
export default function CourseInlineContentCreator({ courseId, type, language, onCancel, onSaved }: Props) {
  ...
  const [info, setInfo] = useState({
    title: "",
    description: "",
    subject: language === 'ar' ? "اللغة العربية" : "Arabic",
  ...
  const labels = {
    ...
    itemPlaceholder: isQuiz 
      ...
      : (language === 'ar' ? "اكتب المطلوب من الطالب" : "Write student instructions"),
    empty: isQuiz 
      ? (language === 'ar' ? "لا توجد أسئلة بعد" : "No questions yet")
      : (language === 'ar' ? "لا توجد بنود تكليف بعد" : "No assignment items yet"),
    addItem: isQuiz 
  ...
  const addItem = () => {
    ...
    const itemToSave = currentItem.type === "TRUE_FALSE"
      ? { ...currentItem, options: language === 'ar' ? ["صحيح", "خطأ", "", ""] : ["True", "False", "", ""] }
      : currentItem;

    setItems([...items, itemToSave]);
    setCurrentItem(createEmptyItem(type, language));
    showToast(isQuiz ? (language === 'ar' ? "تمت إضافة السؤال بنجاح" : "Question added successfully") : (language === 'ar' ? "تمت إضافة بند التكليف بنجاح" : "Assignment item added successfully"), "success");
  };
  ...
  const saveContent = async () => {
    ...
    if (items.length === 0) {
      showToast(isQuiz ? (language === 'ar' ? "يرجى إضافة سؤال واحد على الأقل قبل الحفظ" : "Please add at least one question") : (language === 'ar' ? "يرجى إضافة بند تكليف واحد على الأقل قبل الحفظ" : "Please add at least one item"), "error");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        questions: items.map((item, index) => ({
          ...item,
          options: item.type === "TRUE_FALSE" ? (language === 'ar' ? ["صحيح", "خطأ"] : ["True", "False"]) : item.options.filter(Boolean),
      ...
      showToast(error.message || (language === 'ar' ? "حدث خطأ أثناء الحفظ" : "An error occurred while saving"), "error");
    } finally {
      setSaving(false);
    }
  };
