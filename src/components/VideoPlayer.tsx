...
function getVideoInfo(url: string): { type: 'youtube' | 'vimeo' | 'direct', id: string } | null {
  if (!url) return null;
  const normalized = url.trim();

  // Direct video files (.mp4, .webm, .ogg, .mov, blob:, data:video)
  if (/\.(mp4|webm|ogg|mov)($|\?)/i.test(normalized) || normalized.startsWith("blob:") || normalized.startsWith("data:video")) {
  ...
  // Fallback if URL is standard HTTP/HTTPS
...
export default function VideoPlayer({ url, onProgress, onDuration }: VideoPlayerProps) {
  ...
  if (!url || !url.trim() || !videoType || !videoId) {
    return null;
  }
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full bg-black flex flex-col items-center justify-center overflow-hidden">
      {videoType === 'youtube' && (
        <iframe
          ref={youtubeRef}
          src={`https://www.youtube-nocookie.com/embed/${videoId}?enablejsapi=1&origin=${origin}&rel=0&autoplay=0&modestbranding=1&controls=1&showinfo=0&iv_load_policy=3&fs=1`}
      ...
      {videoType === 'direct' && (
        <video
          ...
          onTimeUpdate={(e: any) => reportProgress(e.target.currentTime)}
          onLoadedMetadata={(e: any) => reportDuration(e.target.duration)}
        />
      )}
    </div>
  );
