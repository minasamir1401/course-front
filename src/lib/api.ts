...
/** Auth is handled by httpOnly cookie — token is not stored in localStorage */
export const getActiveToken = (): string | null => {
  return null; // Cookie is sent automatically by the browser via credentials: 'include'
};
...
const clearSessionAndRedirect = () => {
  ...
  // Remove all possible token keys (including legacy sessions that stored token in localStorage)
  Object.values(TOKEN_CONFIG).forEach(c => {
    localStorage.removeItem(c.token);
    localStorage.removeItem(`${c.token}_expires_at`);
  });
