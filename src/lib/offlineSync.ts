...
// Token is in httpOnly cookie — sent automatically via credentials: 'include'
// No need to read from localStorage anymore
function getCurrentAuthorization(): string | null {
  return null; // Cookie handles auth — apiFetch adds credentials: 'include'
}
...
class OfflineSyncManager {
  ...
  private async flushQueue() {
    ...
    for (const item of [...this.queue]) {
      try {
        // Strip any stale Authorization headers from queued items (cookie handles auth now)
