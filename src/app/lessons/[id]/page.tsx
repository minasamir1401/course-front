...
export default function LessonPlayerPage() {
    ...
    return (
      <div className="min-h-screen bg-[#05050a] flex items-center justify-center">
        <div className="relative">
          ...
          <div className="mt-8 text-indigo-400 font-black animate-pulse text-center">جاري تحميل الدرس...</div>
        </div>
      </div>
    );
  }

  if (!lesson) return <div className="text-white p-20 text-center font-black">الدرس غير موجود</div>;
  ...
  return (
    <div className="min-h-screen bg-[#05050a] text-slate-200 overflow-hidden flex flex-col" dir="rtl">
      ...
      <main className="flex-1 relative flex items-center justify-center overflow-hidden">
        ...
        {currentStage === 'slides' && (
          <div className="w-full h-full flex flex-col md:flex-row p-6 gap-6">
            ...
            {/* Side Media Panel (Video or Summary) */}
        ...
        {currentStage === 'exercises' && (
          <div className="max-w-4xl w-full p-8 animate-in slide-in-from-bottom-8 duration-700">
                     ...
                     <button 
                       ...
                       className={`w-12 h-12 rounded-xl font-black transition-all border-2 ${
                         currentQuestionIndex === i 
                          ? 'bg-indigo-600 border-white text-white shadow-xl shadow-indigo-600/20' 
                          : answers[i] 
             ...
             {/* Question Card */}
             <div className="bg-[#0f0f1d] border border-white/5 rounded-[50px] p-12 shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-600/5 blur-[100px] -mr-32 -mt-32"></div>
                
                <div className="relative z-10">
                   <div className="flex items-center gap-3 mb-6 flex-wrap">
                      <span className="bg-indigo-600/10 text-indigo-400 px-3 py-1 rounded-lg text-[10px] font-black uppercase border border-indigo-500/20">
                         {lesson.questions[currentQuestionIndex].type || "MCQ"}
                      </span>
                      <span className="text-slate-600 text-xs font-bold">•</span>
                      <span className="text-slate-500 text-xs font-bold">ناتج التعلم: {lesson.questions[currentQuestionIndex].learningOutcome || "N/A"}</span>
                      <span className="text-slate-600 text-xs font-bold">•</span>
                      <span className="text-emerald-500 text-xs font-bold">المهارة: {lesson.questions[currentQuestionIndex].skill || "General"}</span>
                      <span className="text-slate-600 text-xs font-bold">•</span>
                      <span className="text-amber-500 text-xs font-bold">الدرجة: {lesson.questions[currentQuestionIndex].points || 1} نقطة</span>
                   </div>

                   <h3 
                     className="text-2xl md:text-3xl font-black text-white mb-10 leading-relaxed"
                     dangerouslySetInnerHTML={{ __html: lesson.questions[currentQuestionIndex].text }}
                   />
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                      {lesson.questions[currentQuestionIndex].options.map((opt: string, oIdx: number) => {
                        ...
                        let cardStyle = "bg-white/5 border-transparent hover:bg-white/[0.08] hover:border-white/10";
                        if (isSelected) {
                          cardStyle = "bg-indigo-600 border-white text-white shadow-xl shadow-indigo-600/20 scale-[1.02]";
                        }
                        ...
                        return (
                          <button 
                            ...
                            className={`p-6 rounded-[30px] border-2 text-right transition-all duration-300 font-black text-lg flex items-center justify-between group/opt ${cardStyle}`}
                          >
                             <span>{opt}</span>
                   ...
                   <div className="flex justify-between items-center mt-12">
                      <button 
                        ...
                        className="bg-white/5 text-white px-8 py-4 rounded-2xl font-black hover:bg-white/10 transition-all flex items-center gap-3 disabled:opacity-30"
                      >
                      ...
                      <button 
                        ...
                        className={`px-10 py-4 rounded-2xl font-black hover:scale-105 transition-all flex items-center gap-3 ${currentQuestionIndex < lesson.questions.length - 1 ? 'bg-white text-black shadow-xl shadow-white/5' : 'bg-emerald-600 text-white shadow-xl shadow-emerald-600/20'}`}
                      >
        ...
        {currentStage === 'summary' && (
          <div className="max-w-4xl w-full p-8 animate-in zoom-in duration-700 text-center">
             ...
             <p className="text-slate-500 text-xl font-bold mb-12">لقد أتممت الدرس والتدريبات بنجاح باهر.</p>

             <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                <div className="bg-white/[0.02] border border-white/5 p-8 rounded-[40px]">
                   <p className="text-slate-500 text-sm font-black mb-2 uppercase">النتيجة النهائية</p>
                   <p className="text-4xl font-black text-white">{score} / {lesson.questions.reduce((acc: number, q: any) => acc + (q.points || 1), 0)}</p>
                </div>
                <div className="bg-white/[0.02] border border-white/5 p-8 rounded-[40px]">
                   <p className="text-slate-500 text-sm font-black mb-2 uppercase">نسبة الإنجاز</p>
                   <p className="text-4xl font-black text-indigo-400">{Math.round((score / Math.max(1, lesson.questions.reduce((acc: number, q: any) => acc + (q.points || 1), 0))) * 100)}%</p>
                </div>
                <div className="bg-white/[0.02] border border-white/5 p-8 rounded-[40px]">
                   <p className="text-slate-500 text-sm font-black mb-2 uppercase">المستوى الأكاديمي</p>
                   <p className="text-4xl font-black text-emerald-400">ممتاز</p>
                </div>
             </div>
             ...
             <div className="flex flex-col sm:flex-row gap-6 justify-center">
                <button 
                  ...
                  className="bg-white/5 text-white border border-white/10 px-12 py-5 rounded-[22px] font-black text-lg hover:bg-white/10 transition-all flex items-center gap-3"
                >
                ...
                <button 
                  ...
                  className="bg-indigo-600 text-white px-12 py-5 rounded-[22px] font-black text-lg hover:scale-105 transition-all shadow-2xl shadow-indigo-600/20 flex items-center gap-3"
                >
                   العودة للكورس
                   <ChevronLeft className="w-6 h-6" />
                </button>
      ...
      {/* Sidebar for Attachments/Notes (Collapsible) */}
      <aside className="fixed left-0 top-20 bottom-0 w-80 bg-[#0a0a14] border-r border-white/5 transform -translate-x-full lg:translate-x-0 transition-transform duration-500 z-40 p-8 flex flex-col gap-8 shadow-2xl">
               ...
               {lesson.attachments?.map((att: any, i: number) => (
                 <a key={i} href={att.url} target="_blank" className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-indigo-500/30 transition-all group">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-xs font-black text-slate-300 group-hover:text-white truncate max-w-[120px]">{att.name || "ملف"}</span>
                    </div>
                    <span className="text-[10px] font-black text-slate-500 uppercase">{att.type}</span>
                 </a>
               ))}
         ...
         <div className="flex-1 space-y-6 overflow-y-auto custom-scrollbar">
            ...
            <div className="bg-white/5 border border-white/5 p-6 rounded-[30px] text-xs font-bold text-slate-400 leading-relaxed">
               {lesson.notes || "لا توجد ملاحظات إضافية لهذا الدرس."}
            </div>
         </div>

         <div className="p-6 bg-indigo-600/10 border border-indigo-500/20 rounded-[35px]">
            <div className="flex items-center gap-3 mb-4">
               <Award className="w-5 h-5 text-indigo-400" />
               <h5 className="text-sm font-black text-white">إنجازك في الوحدة</h5>
            </div>
            <div className="w-full bg-black/40 h-2 rounded-full overflow-hidden mb-2">
               <div className="h-full bg-indigo-500 w-3/4"></div>
            </div>
            <p className="text-[10px] font-black text-slate-500">تم إنجاز 75% من المسار التعليمي</p>
         </div>
      </aside>
