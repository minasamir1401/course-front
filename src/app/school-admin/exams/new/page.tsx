...
export default function SchoolAdminNewExamPage() {
  ...
  const moduleMode = requestedMode === "module" || (!requestedMode && pathname === "/school-admin/exams/new");
  ...
  const openDraftExamEditor = (index: number) => {
    ...
    const persistDraftAndOpen = async () => {
      ...
      if ((createdIdRef.current || createdId) && targetModule.id && localSubExam.id) {
        router.push(
          `/school-admin/exams/edit/${createdIdRef.current || createdId}?moduleId=${encodeURIComponent(targetModule.id)}&subExamId=${encodeURIComponent(localSubExam.id)}`
        );
        return;
      }
      ...
      try {
        ...
        router.push(
          `/school-admin/exams/edit/${savedExamId}?moduleId=${encodeURIComponent(savedModule.id)}&subExamId=${encodeURIComponent(savedSubExam.id)}`
        );
      } catch (error: any) {
