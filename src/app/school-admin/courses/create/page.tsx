...
export default function CreateCoursePage() {
  ...
  useEffect(() => {
    ...
    const snapshot = JSON.stringify({ createdId, courseData, lessons, isLessonModalOpen, currentLesson, editingLessonIndex });
    ...
    const timer = setTimeout(() => {
      ...
      try {
        ...
        const payload = {
          title: courseData.title || (language === 'ar' ? "مسودة كورس بدون عنوان" : "Untitled Course Draft"),
          description: courseData.description,
          coverImage: courseData.coverImage || null,
          grades: courseData.grades,
          subject: subjectString || (language === 'ar' ? "غير محدد" : "Unspecified"),
        ...
        const activeCourseId = createdIdRef.current;
        const method = activeCourseId ? "PUT" : "POST";
        const url = activeCourseId
          ? `${API_URL}/school/courses/${activeCourseId}`
  ...
  }, [isAutoSaveEnabled, isLoading, createdId, courseData, lessons, isLessonModalOpen, currentLesson, editingLessonIndex]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // A publish is always ordered after the last autosave. This prevents an older
      // request from committing after the final, newer course snapshot.
      ...
      <div dir={language === 'ar' ? 'rtl' : 'ltr'}>
        {isLessonModalOpen ? (
          <div className="max-w-6xl mx-auto w-full h-[100dvh] sm:h-[calc(100dvh-3rem)] animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white border border-slate-200 w-full h-full rounded-none sm:rounded-[40px] shadow-2xl overflow-hidden flex flex-col">
              {/* Modal Header */}
              <div className="bg-slate-900 p-3 sm:p-8 flex justify-between items-center gap-3">
                <div className="min-w-0">
                  ...
                  <p className="hidden sm:block text-slate-400 mt-1 font-bold">{language === 'ar' ? "بناء محتوى الدرس والأهداف والمهام التفاعلية" : "Build lesson content, objectives, and interactive tasks"}</p>
                </div>
                          ...
                          <div className="relative">
                            ...
                            {isIndicatorDropdownOpen && (
                              <>
                                ...
                                <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl max-h-72 overflow-y-auto p-3 space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
                                  {[
                                    language === 'ar' ? "Indicator 1: Identifies Basic Concepts" : "Indicator 1: Identifies core concepts",
                                    language === 'ar' ? "Indicator 2: Applies Mathematical Laws" : "Indicator 2: Applies mathematical laws",
                                    language === 'ar' ? "Indicator 3: Infers Relationships" : "Indicator 3: Infers relationships"
                                  ].map((option) => {
                                    const selected = (currentLesson.indicators || "").split("\n").filter(Boolean);
                                    const isSelected = selected.includes(option);
                                    return (
                                  ...
                                  {(() => {
                                    ...
                                    const defaultOptions = [
                                      ...
                                      language === 'ar' ? "Indicator 2: Applies Mathematical Laws" : "Indicator 2: Applies mathematical laws",
                                      language === 'ar' ? "Indicator 3: Infers Relationships" : "Indicator 3: Infers relationships",
                                      "Indicator 1: Identifies Basic Concepts",
                                      "Indicator 2: Applies Mathematical Laws",
                                      "Indicator 3: Infers Relationships"
                                    ];
                                    const customOpts = selected.filter((x: string) => !defaultOptions.includes(x));
                                    return customOpts.map((option: string) => (
                                      <div key={option} className="flex items-center justify-between gap-2 px-3 py-1 hover:bg-slate-50 rounded-xl text-slate-700 font-bold text-xs">
                          ...
                          <div className="relative">
                            ...
                            {isOutcomeDropdownOpen && (
                              <>
                                ...
                                <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl max-h-72 overflow-y-auto p-3 space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
                                  {[
                                    language === 'ar' ? "Outcome 1: Student will be able to..." : "Outcome 1: Student will be able to...",
                                    language === 'ar' ? "Outcome 2: Student will distinguish between..." : "Outcome 2: Student will distinguish between...",
                                    language === 'ar' ? "Outcome 3: Student will analyze..." : "Outcome 3: Student will analyze..."
                                  ].map((option) => {
                                    const selected = (currentLesson.learningOutcomes || "").split("\n").filter(Boolean);
                                    const isSelected = selected.includes(option);
                                    return (
                                  ...
                                  {(() => {
                                    ...
                                    const defaultOptions = [
                                      ...
                                      language === 'ar' ? "Outcome 2: Student will distinguish between..." : "Outcome 2: Student will distinguish between...",
                                      language === 'ar' ? "Outcome 3: Student will analyze..." : "Outcome 3: Student will analyze...",
                                      "Outcome 1: Student will be able to...",
                                      "Outcome 2: Student will distinguish between...",
                                      "Outcome 3: Student will analyze..."
                                    ];
                                    const customOpts = selected.filter((x: string) => !defaultOptions.includes(x));
                                    return customOpts.map((option: string) => (
                                      <div key={option} className="flex items-center justify-between gap-2 px-3 py-1 hover:bg-slate-50 rounded-xl text-slate-700 font-bold text-xs">
