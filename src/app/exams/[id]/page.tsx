  ...
  useEffect(() => {
    if (exam && !isPreviewMode) {
      try {
        ...
        const savedTime = localStorage.getItem(`exam_${id}_time`);
    ...
    if (started && timeLeft > 0) {
      const timer = setInterval(() => {
        setTimeLeft((prev) => {
          ...
          if (!isPreviewMode) localStorage.setItem(`exam_${id}_time`, newTime.toString());
          return newTime;
        });
      }, 1000);
    ...
    try {
      ...
      if (res.ok) {
        if (!isPreviewMode) {
          ...
          localStorage.removeItem(`exam_${id}_time`);
        }
        if (isPreviewMode) {
                ...
                <>
                  {question.level && (
                    <InteractiveTag 
                      ...
                      value={question.level === 'Easy' || question.level === 'Foundation' ? (language === 'ar' ? 'تأسيسي' : 'Foundation') : question.level === 'Medium' || question.level === 'On Level' ? (language === 'ar' ? 'في المستوى' : 'On Level') : (language === 'ar' ? 'متقدم' : 'Advanced')}
                  ...
                  {question.skill && (
                    <InteractiveTag 
                      label={language === 'ar' ? 'المهارة' : 'Skill'}
                  ...
                  {question.learningOutcome && (
                    <InteractiveTag 
                      label={language === 'ar' ? 'ناتج التعلم' : 'Outcome'}
