...
import { Lock, User, BookOpen, GraduationCap, Eye, EyeOff } from 'lucide-react';
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 502 || res.status === 503 || res.status === 504) {
          setError(language === 'ar' ? 'خادم المنصة لا يستجيب حالياً. يرجى المحاولة لاحقاً.' : 'Server is currently unreachable. Please try again later.');
        } else {
      ...
      if (data.user.role === "TEACHER") {
        ...
        localStorage.setItem("school_admin_token_expires_at", String(data.expiresAt || Date.now() + (8 * 60 * 60 * 1000)));
        ...
        localStorage.setItem("lms_token_expires_at", String(data.expiresAt || Date.now() + (8 * 60 * 60 * 1000)));
        localStorage.setItem(
          "lms_user",
          JSON.stringify({
            id: data.user.id,
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full ${language === 'ar' ? 'pl-10 pr-10' : 'pr-10 pl-10'} py-3 rounded-xl outline-none transition-all text-white placeholder-slate-600`}
                    style={{
