    ...
    const fetchStats = async () => {
      try {
        ...
        if (user.role === "TEACHER") {
          setIsLoading(false);
          return; // No need to fetch student stats for teacher yet
        }
      ...
      <div className="w-full max-w-[1600px] mx-auto space-y-4 md:space-y-12 pb-24 px-1 sm:px-2 md:px-4 overflow-x-hidden" dir={language === 'ar' ? "rtl" : "ltr"}>
        ...
        <div className="relative overflow-hidden rounded-[32px] md:rounded-[48px] premium-gradient-primary shadow-2xl shadow-indigo-500/20 group">
          ...
          <div className="relative z-10 px-4 py-6 md:py-14 lg:py-16 xl:py-24 lg:px-14 xl:px-20 flex flex-col xl:flex-row items-center justify-between gap-6 xl:gap-12">
              ...
              <div className="flex flex-col xs:flex-row flex-wrap justify-center xl:justify-start gap-2 md:gap-5 pt-2 w-full">
                ...
                <div className="glass px-6 md:px-8 py-4 md:py-5 rounded-2xl md:rounded-3xl flex items-center gap-4 md:gap-5 border-white/10">
                   ...
                   <div className="text-start">
                      <p className="text-indigo-100 text-[8px] md:text-[10px] font-black uppercase tracking-[1px] md:tracking-[2px]">{t('dashboard.currentLevel')}</p>
            ...
            {/* 3D Progress Widget – hidden on lg, visible on xl+ */}
            <div className="relative group w-full max-w-[280px] hidden xl:block">
               ...
               <div className="relative glass p-6 md:p-10 rounded-[40px] md:rounded-[60px] border-white/20 shadow-2xl flex flex-col items-center justify-center w-full floating">
                  <div className="relative w-32 h-32 md:w-44 md:h-44 mb-6 md:mb-8">
                     ...
                     <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
                        ...
                        <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest opacity-60">{t('dashboard.overallProgress')}</span>
                     </div>
                  </div>
                  <div className="flex gap-3 md:gap-4">
                     <div className="px-4 md:px-5 py-2 md:py-3 bg-white/10 rounded-xl md:rounded-2xl border border-white/10 text-center">
                        ...
                        <p className="text-[8px] md:text-[9px] text-white/60 font-black uppercase tracking-widest">{t('dashboard.coursesCount')}</p>
                     </div>
                     <div className="px-4 md:px-5 py-2 md:py-3 bg-white/10 rounded-xl md:rounded-2xl border border-white/10 text-center">
                        <p className="text-white font-black text-base md:text-lg">{stats?.totalExams || 0}</p>
                        <p className="text-[8px] md:text-[9px] text-white/60 font-black uppercase tracking-widest">{t('dashboard.examsCount')}</p>
                     </div>
                  </div>
               </div>
            </div>
          ...
          <div className="lg:col-span-8 space-y-8 xl:space-y-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 px-2">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{t('dashboard.yourCourses')}</h2>
                    <p className="text-slate-400 font-bold text-xs md:text-sm">{t('dashboard.continueWhereYouLeft')}</p>
                 </div>
              </div>
              <button
                onClick={() => router.push('/courses')}
                className="group flex items-center gap-3 text-indigo-600 font-black text-sm transition-all"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {stats?.courseProgresses?.length > 0 ? stats.courseProgresses.slice(0, 4).map((course: any, i: number) => (
                ...
                >
                  <div className="relative h-48 md:h-60 w-full overflow-hidden">
                    ...
                    <div className="absolute bottom-4 md:bottom-6 right-4 md:right-6 left-4 md:left-6 flex justify-between items-end">
                       <div className="space-y-1">
                          <p className="text-white/60 text-[9px] md:text-[10px] font-black uppercase tracking-widest">{t('dashboard.level')}</p>
                          <p className="text-white font-black text-sm md:text-base">{t('dashboard.level')} {i + 1}</p>
                       </div>
                    ...
                    <div className="space-y-2 md:space-y-3">
                       ...
                       <div className="flex items-center gap-4 md:gap-6">
                          <div className="flex items-center gap-1.5 text-slate-400 font-bold text-[10px] md:text-xs">
                             ...
                             {course.totalLessons || 0} {t('dashboard.lessons')}
                          </div>
                          <div className="flex items-center gap-1.5 text-slate-400 font-bold text-[10px] md:text-xs">
                             <Target className="w-3.5 h-3.5 md:w-4 md:h-4 text-amber-500" />
                             {Math.ceil((course.totalLessons || 0) * 0.2)} {t('dashboard.examsCount')}
                          </div>
                       </div>
                    </div>
                ...
                <div className="col-span-full py-32 text-center bg-white rounded-[60px] border-4 border-dashed border-slate-100">
                  ...
                  <p className="text-slate-900 text-2xl font-black mb-2">{t('dashboard.startJourney')}</p>
                </div>
              )}
            </div>
          </div>
               ...
               <div className="space-y-5 relative z-10">
                 {stats?.recentExams?.length > 0 ? stats.recentExams.map((submission: any, i: number) => (
                   <div key={i} className="flex gap-4 md:gap-5 items-center p-4 md:p-5 rounded-[25px] md:rounded-[35px] hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 group/item">
                     ...
                     <div className="min-w-0 flex-1 text-start">
                       ...
                       <div className="flex items-center gap-2 md:gap-3 justify-start">
                         <div className="px-2 md:px-3 py-1 bg-emerald-50 text-emerald-600 text-[7px] md:text-[8px] font-black rounded-lg uppercase tracking-widest border border-emerald-100">{t('dashboard.passed')} ✅</div>
                         <div className="flex items-center gap-1.5 text-slate-400 font-bold text-[8px] md:text-[9px]">
                            <Calendar className="w-2.5 h-2.5 md:w-3 md:h-3" />
                            {new Date(submission.date).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US')}
                         </div>
                       </div>
                     </div>
                   </div>
                   ...
                   <div className="py-20 text-center space-y-4">
                     <p className="text-slate-400 text-sm font-bold">{t('dashboard.noResults')}</p>
                   </div>
                 )}
               </div>
            </div>
               ...
               <div className="space-y-6 relative z-10">
                  ...
                  <div className="glass-dark p-6 md:p-8 rounded-[30px] md:rounded-[40px] space-y-6">
                     ...
                     <p className="text-[10px] md:text-[11px] text-white/60 font-bold leading-relaxed">
                        {t('dashboard.hoursLeft')} <span className="text-white">2.5 {t('dashboard.hours')}</span> {t('dashboard.toAchieveGoal')}
                     </p>
                  </div>
               </div>
            </div>
