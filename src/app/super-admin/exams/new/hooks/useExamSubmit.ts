...
export const useExamSubmit = (props: any) => {
  const { examData, modules, isModuleModalOpen, currentModule, editingModuleIndex, manualSubmitRef, autoSaveGenerationRef, autoSaveTimerRef, setIsLoading, autoSaveWriteQueueRef, createdIdRef, standaloneQuestions, deletedQuestionIds, setDeletedQuestionIds, showToast, language, router, t, isLoading, moduleMode } = props;
    ...
    if (!moduleMode && !examData.title) {
      showToast(language === 'ar' ? "يرجى إدخال عنوان الاختبار في الإعدادات" : "Please enter the exam title in Settings", "error");
      return;
    }
    if (!moduleMode && (!examData.duration || examData.duration <= 0)) {
      showToast(language === 'ar' ? "يرجى تحديد مدة الاختبار في الإعدادات" : "Please specify the exam duration in Settings", "error");
      return;
    }
    if (!moduleMode && (examData.passingScore === undefined || examData.passingScore === null || examData.passingScore < 0)) {
      showToast(language === 'ar' ? "يرجى تحديد درجة النجاح في الإعدادات" : "Please specify the passing score in Settings", "error");
      return;
    }

    if (!moduleMode && (!examData.grades || examData.grades.length === 0)) {
      showToast(language === 'ar' ? "يرجى تحديد الصفوف الدراسية في الإعدادات" : "Please select grades in Settings", "error");
      return;
    }
    if (!moduleMode && (!examData.subjects || examData.subjects.length === 0)) {
      showToast(language === 'ar' ? "يرجى تحديد مجال التخصص في الإعدادات" : "Please select subject specialization in Settings", "error");
      return;
    }


    if (isModuleModalOpen && (!currentModule || !currentModule.title)) {
      showToast(language === 'ar' ? "يرجى إدخال عنوان الموديول" : "Please enter the module title", "error");
      return;
    }
