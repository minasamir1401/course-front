...
import { SECTION_STYLE_PRESETS } from '../constants';
  ...
  export const SlidesBuilder = (props: any) => {
  const { openDropdownId, setOpenDropdownId, allExistingSkills, setCustomSkills, addBlock, insertBlockAt, updateBlock, updateBlockTypeAndReset, moveBlock, removeBlock, addSection, removeSection, updateSection } = props;
  const { source, currentModule, language, t, editingModuleIndex, setModules, modules, setCurrentModule, tempQuestion, setTempQuestion, setIsQuestionIndicatorOpen, setIsQuestionOutcomeOpen, setIsQuestionStandardOpen } = props;
  const [isQuestionIndicatorOpen, setLocalIsQuestionIndicatorOpen] = React.useState(false);
  const [isQuestionOutcomeOpen, setLocalIsQuestionOutcomeOpen] = React.useState(false);
  const [isQuestionStandardOpen, setLocalIsQuestionStandardOpen] = React.useState(false);
