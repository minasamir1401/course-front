...
export default function SuperAdminNewExamPage() {
  ...
  const updatePrimaryModule = (updater: (module: any) => any) => {
    const baseModule = primaryModule || {
      id: String(Date.now()),
      title: examData.title || (language === 'ar' ? 'موديول بدون عنوان' : 'Untitled Module'),
      subExams: [],
      questions: [],
      slides: [],
      assignments: []
    ...
    const nextModule = updater(baseModule);
    setModules((currentModules: any[]) => {
      if (!currentModules.length) return [nextModule];
  ...
  const openDraftExamEditor = (index: number) => {
    const targetModule = primaryModule || currentModule;
    if (!targetModule) return;
    const persistDraftAndOpen = async () => {
      const localSubExam = (targetModule.subExams || [])[index];
      if (!localSubExam) return;

      if ((createdIdRef.current || createdId) && targetModule.id && localSubExam.id) {
        router.push(
          `/super-admin/exams/edit/${createdIdRef.current || createdId}?moduleId=${encodeURIComponent(targetModule.id)}&subExamId=${encodeURIComponent(localSubExam.id)}`
        );
        return;
      }
        ...
        <div className="animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className={`min-w-0 space-y-8 ${showSettings ? 'lg:col-span-8' : 'lg:col-span-12'}`}>
              {moduleMode ? (
                <div className="rounded-[36px] bg-white border border-slate-100 shadow-sm p-7 space-y-6">
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between border-b border-slate-100 pb-5">
                    <div>
                      <div className="text-xs font-black uppercase tracking-[0.22em] text-indigo-600 mb-1">
                        {language === 'ar' ? 'الاختبارات داخل الموديول' : 'Module Exams'}
                      </div>
                      <h3 className="text-2xl font-black text-slate-900">
                        {examData.title || primaryModule?.title || (language === 'ar' ? 'موديول بدون عنوان' : 'Untitled Module')}
                      </h3>
                      <p className="mt-1 text-sm font-bold text-slate-400">
                        {(primaryModule?.subExams || []).length} {language === 'ar' ? 'اختبارات مضافة' : 'exams added'}
                      </p>
                    </div>
                    <div className="flex w-full gap-2 md:w-auto">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black text-white transition-all hover:bg-indigo-700 cursor-pointer shrink-0 shadow-md shadow-indigo-100"
                      >
                            ...
                            <div key={subExam.id || index} className="grid grid-cols-1 gap-4 px-5 py-5 md:grid-cols-[minmax(0,1.5fr)_160px_180px] md:items-center">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                <button
                                  ...
                                  className="inline-flex items-center gap-2 rounded-xl bg-indigo-50 px-4 py-2.5 text-sm font-black text-indigo-700 transition-all hover:bg-indigo-100 cursor-pointer"
                                >
                                ...
                                <button
                                  ...
                                  className="inline-flex items-center gap-2 rounded-xl bg-red-50 px-4 py-2.5 text-sm font-black text-red-600 transition-all hover:bg-red-100 cursor-pointer"
                                >
