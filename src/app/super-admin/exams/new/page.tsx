...
export function SuperAdminNewExamPageContent({ presetType, presetCourseId }: { presetType?: 'Exam' | 'Quiz' | 'Assignment', presetCourseId?: string }) {
  ...
  const fetchSchools = async () => {
    ...
    try {
      ...
      if (schoolsRes.ok) {
        ...
        setFetchError(language === 'ar' ? "فشل في جلب قائمة المدارس" : "Failed to fetch schools");
      }
    } catch (e) {
      console.error(e);
      setFetchError(language === 'ar' ? "خطأ في الاتصال بالسيرفر" : "Server connection error");
    } finally {
  ...
  const updateOption = (oIndex: number, value: string) => {
    ...
    } else if (updatedQuestion.correctAnswer === oldOptionValue && oldOptionValue !== "") {
  ...
  const updateCorrectAnswers = (optionIndex: number) => {
    ...
    if (question.type === "TRUE_FALSE") {
      question.correctAnswer = optionIndex === 0 ? (language === 'ar' ? "صحيح" : "True") : (language === 'ar' ? "خطأ" : "False");
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const payload = {
          ...
          category: examInfo.subjects?.[0] || (language === 'ar' ? "غير محدد" : "Unspecified"),
          status: "DRAFT",
        ...
        if (res.ok) {
          ...
          // Fix: Update IDs only to avoid overwriting ongoing edits
          ...
          showToast(language === 'ar' ? "تم حذف هذا الاختبار من مكان آخر. تم إيقاف الحفظ التلقائي." : "This exam was deleted elsewhere. Auto-save disabled.", "error");
        } else {
  ...
  const handleSubmit = async (status: string = "PUBLISHED") => {
    if (!examInfo.title) {
      showToast(language === 'ar' ? "يرجى إدخال عنوان الاختبار" : "Please enter the exam title", 'error');
      return;
    }

    if (!examInfo.subjects || examInfo.subjects.length === 0) {
      showToast(language === 'ar' ? "يرجى اختيار مادة واحدة على الأقل" : "Please select at least one subject", 'error');
      return;
    }

    if (questions.length === 0) {
      showToast(language === 'ar' ? "يرجى إضافة سؤال أو شريحة واحدة على الأقل" : "Please add at least one question or slide", 'error');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        showToast(status === "DRAFT" ? (language === 'ar' ? "تم حفظ المسودة بنجاح!" : "Draft saved successfully!") : (language === 'ar' ? "تم نشر الاختبار بنجاح!" : "Exam published successfully!"), 'success');
        if (courseIdParam) {
        ...
        let errMessage = language === 'ar' ? "فشل في إضافة الاختبار" : "Failed to add exam";
        try {
          const err = await res.json();
          errMessage = err.error || errMessage;
        } catch (e) {
          if (res.status === 413) errMessage = language === 'ar' ? "حجم البيانات كبير جداً (Payload Too Large). يرجى تقليل حجم الصور المستخدمة." : "Payload Too Large. Please reduce image sizes.";
      ...
      showToast(language === 'ar' ? "حدث خطأ غير متوقع. يرجى التحقق من اتصالك." : "An unexpected error occurred. Please check your connection.", 'error');
    } finally {
      setSaving(false);
    }
  };
                  ...
                  <div className="p-8 md:p-12 space-y-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-slate-50 border border-slate-200 rounded-[30px] shadow-sm mb-6">
                      ...
                      {currentQuestion.type !== 'TEXT' && (
                        ...
                        <div className="flex flex-col gap-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">XP</label>
                          <input
                                type="number"
                                className="bg-white border border-slate-200 rounded-xl px-3 py-2 font-bold text-slate-700 text-xs outline-none min-h-[34px]"
                    ...
                    <div className="flex flex-col gap-5 border-t border-slate-100 pt-6">
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-slate-400 text-[10px] font-bold mt-0.5">{language === 'ar' ? 'أضف شرحاً تفصيلياً يظهر بعد تسليم الإجابة في تقرير الاختبار' : 'Add a detailed explanation to appear after submitting the answer in the exam report'}</p>
                        </div>
                          ...
                          <>
                            <div className="flex items-center gap-4">
                              ...
                              >
                                ...
                                <span className={`font-black text-lg ${currentQuestion.correctAnswer === 0 ? 'text-emerald-700' : 'text-slate-600'}`}>{language === 'ar' ? "صحيح" : "True"}</span>
                              </div>
                              <div 
                              ...
                              >
                                ...
                                <span className={`font-black text-lg ${currentQuestion.correctAnswer === 1 ? 'text-rose-700' : 'text-slate-600'}`}>{language === 'ar' ? "خطأ" : "False"}</span>
                              </div>
                            </div>
                          </>
...
return (
                        ...
                        return (
                          <label key={g} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border cursor-pointer transition-all ${selected ? 'bg-indigo-100 border-indigo-300 text-indigo-900 shadow-sm' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                            <input type="checkbox" className="hidden" checked={selected} onChange={(e) => {
                              if (e.target.checked) {
                                setExamInfo({...examInfo, grades: Array.from(new Set([...(examInfo.grades || []), g]))});
                              } else {
                                setExamInfo({...examInfo, grades: (examInfo.grades || []).filter((gr: string) => gr !== g && gr !== enLabel)});
                              }
                            }} />
      ...
      {previewQuestion && (
        <div className="fixed inset-0 z-[100] bg-white w-full h-full flex flex-col overflow-hidden animate-in fade-in duration-300 ltr" dir="ltr">
            {/* Modal Header */}
               ...
               <div className="max-w-2xl mx-auto space-y-12 relative">
                    {previewQuestion.sections && previewQuestion.sections.length > 0 && (
                      <div className="flex flex-wrap gap-2 justify-end mb-4">
                        <ItemSectionsBubbles item={{sections: previewQuestion.sections}} isSubmitted={false} language={language} filterType="HINT_ONLY" />
                      </div>
                    )}
                  ...
                  {previewQuestion.imageUrl && (
                    <img loading="lazy" decoding="async" src={previewQuestion.imageUrl}
                      alt="Question"
                      ...
                      {(previewQuestion.type === "MCQ" || previewQuestion.type === "MULTI_SELECT" ? previewQuestion.options : (language === 'ar' ? ["صحيح", "خطأ"] : ["True", "False"])).filter((o: string) => o).map((option: string, i: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`text-xl font-bold flex-1 ${textClass}`}><HtmlRenderer html={option} tag="span" /></span>
                            {icon}
                          </button>
                        );
                      })}
