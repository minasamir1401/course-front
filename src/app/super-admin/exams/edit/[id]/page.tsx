...
import StandaloneQuestionsToNewExam from "@/components/exams/StandaloneQuestionsToNewExam";
import { buildPrimaryModulePortalHref, buildSubExamEditorHref, getExamWorkflowView } from "@/lib/examModuleView";
import { selectEditableModule, selectEditableSubExamIndex, selectSynchronizedEditorModule } from "@/lib/examEditorSelection";
import { buildExamEditRoleFallbackHref } from "@/lib/examEditRoleFallback";
import { resolveEditModuleAutoOpenIndex } from "@/lib/editModuleAutopen";
...
export default function SuperAdminEditExamPage() {
  ...
  const editModuleId = searchParams.get("editModuleId");
  const view = searchParams.get("view");
  const search = searchParams.toString();
  const roleFallbackHref = buildExamEditRoleFallbackHref({
    requestedRole: "SUPER_ADMIN",
    examId,
    search: search ? `?${search}` : "",
    hasSuperAdminToken: typeof window !== "undefined" ? !!window.localStorage.getItem("super_admin_token") : false,
    hasSchoolAdminSession: typeof window !== "undefined"
      ? !!window.localStorage.getItem("school_admin_token") || !!window.localStorage.getItem("school_admin_user")
      : false,
  });
  ...
  useExamAutosave({ ...state, showToast, language, moduleId, subExamId });
  
  const { handleSubmit } = useExamSubmit({ ...state, showToast, language, router, t, moduleId, subExamId });

  const workflowView = getExamWorkflowView(moduleId, subExamId);
  const didAutoOpenModuleModalRef = useRef(false);
  const lastAutoOpenedEditModuleIdRef = useRef<string | null>(null);
  const didRedirectToModulePortalRef = useRef(false);
  ...
  const synchronizedEditorModule = selectSynchronizedEditorModule(moduleId, currentModule);
  const editableSubExamIndex = selectEditableSubExamIndex(resolvedSubExamIndex, state.activeSubExamIndex);
  const primaryModulePortalHref = buildPrimaryModulePortalHref("SUPER_ADMIN", examId, modules);
  ...
  React.useEffect(() => {
    if (
      ...
      || editModuleId
      || isLoading
      || !primaryModulePortalHref
    ) {
      didRedirectToModulePortalRef.current = false;
      return;
    }
    if (didRedirectToModulePortalRef.current) return;
  ...
  const openSubExamEditor = (moduleRefId?: string | null, subExamRefId?: string | null) => {
    const href = buildSubExamEditorHref("SUPER_ADMIN", examId, moduleRefId, subExamRefId);
    if (href) {
      router.push(href);
    }
  };
  ...
  React.useEffect(() => {
    if (effectiveWorkflowView !== "full-editor" || createModule !== "1" || modules.length > 0) {
  ...
  }, [createModule, effectiveWorkflowView, examId, language, modules.length, setActiveTab, setCurrentModule, setEditingModuleIndex, setIsModuleModalOpen]);

  React.useEffect(() => {
    if (effectiveWorkflowView !== "full-editor" || !editModuleId) {
      lastAutoOpenedEditModuleIdRef.current = null;
      return;
    }
    const moduleIndex = resolveEditModuleAutoOpenIndex({
      workflowView: effectiveWorkflowView,
      editModuleId,
      handledEditModuleId: lastAutoOpenedEditModuleIdRef.current,
      modules,
    });
    if (moduleIndex === null) return;
    lastAutoOpenedEditModuleIdRef.current = String(editModuleId);
  ...
  if (effectiveWorkflowView === "sub-exam-editor" && moduleId && subExamId) {
    return (
      <DashboardLayout>
        <SubExamEditorScreen
          backHref={`/super-admin/exams/edit/${examId}?moduleId=${encodeURIComponent(moduleId)}`}
          examHref={`/exams/${examId}?preview=true&subExamId=${encodeURIComponent(subExamId)}`}
          currentModule={synchronizedEditorModule}
          activeSubExamIndex={editableSubExamIndex}
          setCurrentModule={setCurrentModule}
          renderQuestionsBuilder={renderQuestionsBuilderProps}
          handleSubmit={handleSubmit}
          isLoading={isLoading}
          isResolving={isLoading || !synchronizedEditorModule || editableSubExamIndex < 0}
          language={language}
        />
      </DashboardLayout>
    );
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="animate-in fade-in duration-500">
          <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between mb-12 gap-6 bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm ${language === 'ar' ? 'text-right' : 'text-left'}`}>
            ...
            >
              {isLoading ? (language === 'ar' ? 'جارٍ الحفظ...' : 'Saving...') : (language === 'ar' ? 'تحديث التقييم' : 'Update Exam')}
            ...
            {showSettings ? (
              ...
              <div className="lg:col-span-12 flex justify-end">
                <button type="button" onClick={() => setShowSettings(true)} className="inline-flex items-center gap-2 px-5 py-3 bg-white border border-indigo-100 text-indigo-600 hover:bg-indigo-50 rounded-2xl font-black shadow-sm transition-all">
                  ...
                  {language === 'ar' ? 'تعديل إعدادات التقييم' : 'Edit Assessment Settings'}
                </button>
              </div>
            )}
