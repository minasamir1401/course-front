...
export function SuperAdminEditExamPageContent({ presetType }: { presetType?: 'Exam' | 'Quiz' | 'Assignment' }) {
    ...
    try {
      ...
      if (examRes.ok) {
        setExamInfo({
          ...
          grades: examData.grades ? JSON.parse(examData.grades) : [examData.grade || CANONICAL_GRADES[0]],
        ...
        setQuestions(examData.questions?.map((q: any) => {
          ...
          return {
            ...
            correctAnswers: q.type === 'MULTI_SELECT' ? (Array.isArray(parsedCorrectAnswers) && parsedCorrectAnswers.length > 0 ? parsedCorrectAnswers : []) : []
          };
        }) || []);
      }
    } catch (e) {
      ...
      if (res.ok) {
        if (isAutoSave) {
          ...
          if (courseIdParam || examInfo.courseId) {
            router.push(`/super-admin/courses/edit/${courseIdParam || examInfo.courseId}`);
          } else {
                ...
                <div className="p-8 md:p-12 space-y-8">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <RichTextEditor
                      ...
                      placeholder={language === 'ar' ? 'اكتب نص الشريحة أو السؤال هنا...' : 'Write the slide text or question prompt here...'}
                    />
                  </div>
                      ...
                      {currentQuestion.type === "TRUE_FALSE" ? (
                        ...
                        <>
                          {currentQuestion.options.map((opt: string, oIndex: number) => (
                            <div key={oIndex} className={`flex items-center gap-4 p-5 rounded-[22px] border-2 transition-all ${isCorrectAnswer(currentQuestion, opt, oIndex) && (opt !== "" || currentQuestion.correctAnswerIndex === oIndex) ? 'bg-emerald-50 border-emerald-500 shadow-md' : 'bg-slate-50 border-transparent hover:border-slate-200'}`}>
                              <div
                              ...
                              >
                                {isCorrectAnswer(currentQuestion, opt, oIndex) && (opt !== "" || currentQuestion.correctAnswerIndex === oIndex) && <CheckCircle2 className="w-5 h-5 text-white" />}
                              </div>
                              <MathInput
              ...
              {previewQuestion.type !== "TEXT" && (
                <div className="grid grid-cols-2 gap-4">
                  {(previewQuestion.type === "MCQ" || previewQuestion.type === "MULTI_SELECT"
                    ...
                    let bgClass = "bg-white border-slate-100 hover:border-indigo-600 hover:bg-indigo-50/30";
                    let textClass = "text-slate-700 group-hover:text-indigo-900";
                    let icon = null;

                    if (isSelected) {
                      if (previewHasChecked) {
                        if (isCorrect) {
                    ...
                    } else if (previewHasChecked && isCorrect) {
                      bgClass = "bg-emerald-50/30 border-emerald-300";
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-${language === 'ar' ? 'right' : 'left'} p-6 rounded-3xl border-2 transition-all flex items-center gap-5 group cursor-pointer ${bgClass}`}
                      >
                        ...
                        <span className={`text-xl font-bold flex-1 transition-all ${textClass}`}><HtmlRenderer html={displayLabel} tag="span" /></span>
                        {icon}
                      </button>
                    );
                  })}
