  ...
  return (
    <DashboardLayout>
      <div className="max-w-7xl mx-auto flex flex-col gap-10 pb-20 rtl" dir="rtl">
        ...
        <div className="bg-[#0f172a] p-8 md:p-12 rounded-[40px] shadow-2xl relative overflow-hidden border border-white/5">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-2 gap-3 bg-slate-50 p-2 rounded-2xl border border-slate-100">
                  <button 
                    ...
                    className={`py-4 rounded-xl font-black text-sm transition-all flex items-center justify-center gap-2 ${examInfo.isCentral ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:bg-white'}`}
                  >
                ...
                {!examInfo.isCentral && (
                  <div className="flex flex-col gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                     {fetchingSchools ? (
                      <div className="flex items-center justify-center py-10 gap-3 text-indigo-600">
                        <Loader2 className="w-6 h-6 animate-spin" />
                        <span className="font-bold text-sm">جاري جلب المدارس...</span>
                      </div>
                    ) : (
                      schools.map((school: any) => (
                        <label key={school.id} className={`flex items-center gap-4 p-4 rounded-xl border-2 transition-all cursor-pointer ${examInfo.schoolIds?.includes(school.id) ? 'bg-white border-indigo-500 shadow-sm' : 'bg-white border-transparent opacity-60 hover:opacity-100'}`}>
                          <div className={`w-6 h-6 rounded-md border-2 flex items-center justify-center transition-all ${examInfo.schoolIds?.includes(school.id) ? 'bg-indigo-600 border-indigo-600' : 'bg-slate-100 border-slate-200'}`}>
                            {examInfo.schoolIds?.includes(school.id) && <CheckCircle className="w-4 h-4 text-white" />}
                          </div>
                          <input 
                            type="checkbox" 
                            className="hidden"
            ...
            <div className="bg-white p-8 rounded-[35px] border border-slate-100 shadow-sm flex flex-col gap-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-xs font-black text-slate-300 uppercase tracking-widest">كلمة السر (اختياري)</label>
                    <div className="relative">
                      <input 
                        type="text"
            ...
            <div className="bg-white p-8 rounded-[35px] border border-slate-100 shadow-sm flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select 
                  className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 outline-none font-bold text-slate-700 text-sm focus:ring-2 focus:ring-indigo-500/20 appearance-none transition-all"
                ...
                <p className="text-xs font-bold text-slate-400 mt-2 bg-slate-50/50 p-3 rounded-xl border border-slate-50">
