...
import { ChevronUp, ChevronDown, CheckCircle2, Edit2, Trash2, Plus, FileText, Settings, Activity, MoveUp, MoveDown, Mic, Video, Image as ImageIcon, Layout, Check, HelpCircle, Upload, Download, Target, X, Save, Languages, Loader2, ArrowRightLeft, Lightbulb, BookOpen } from 'lucide-react';
      ...
      <div className="space-y-8 animate-in fade-in duration-300">
        ...
        {movingQuestion && (
          <MoveQuestionModal
            ...
            questionId={String(movingQuestion.id)}
            questionText={movingQuestion.text || movingQuestion.textEn || ''}
            currentExamId={String(props.examId || props.examData?.id || '')}
            onSuccess={(movedId) => {
              setCurrentModule((prev: any) => {
                ...
                if (source === 'questions' && activeSubExamIndex !== null && prev.subExams && prev.subExams[activeSubExamIndex]) {
                  ...
                  const newQuestions = (newSubExams[activeSubExamIndex].questions || []).filter((item: any) => String(item.id) !== String(movedId));
                  newSubExams[activeSubExamIndex] = { ...newSubExams[activeSubExamIndex], questions: newQuestions };
                  return { ...prev, subExams: newSubExams };
                } else {
                  const newList = (prev[source] || []).filter((item: any) => String(item.id) !== String(movedId));
