...
export default function SuperAdminMonitorPage() {
  ...
  // ⚠️ Circuit breaker: stop hammering server when it's down
  ...
  const handleRestore = async (filename: string, lessonId: string, source?: string) => {
    // ⛔ Block restore attempts when server is known to be down
    if (serverDown) {
      alert(language === 'ar' ? "خادم المنصة غير متاح حالياً. حاول مرة أخرى بعد عودة الخادم." : "Server is currently unavailable. Please try again once the server recovers.");
      return;
    }
    ...
    try {
      ...
      // ✅ Reset circuit breaker on success — server is back
      ...
      alert(isAbort
        ? (language === 'ar' ? "انتهى وقت طلب الاستعادة - الخادم بطيء" : "Restore request timed out - server is slow")
        : err.message
      );
    } finally {
  ...
  const loadData = async (silent = false) => {
    ...
    // ⛔ Circuit breaker: if server has failed too many times, stop polling
    if (silent && consecutiveFailuresRef.current >= MAX_CONSECUTIVE_FAILURES) {
      return;
    }
    ...
    try {
      ...
      if (res.status === 502 || res.status === 503 || res.status === 504) {
        ...
        if (consecutiveFailuresRef.current >= MAX_CONSECUTIVE_FAILURES) {
          setServerDown(true);
          setIsAutoRefresh(false); // auto-pause refresh when server is down
        }
        if (!silent) setError(
          language === 'ar'
            ? `خادم المنصة لا يستجيب (${res.status}). تم إيقاف التحديث التلقائي.`
            : `Server unavailable (${res.status}). Auto-refresh paused.`
        );
        return;
      }
      ...
      // ✅ Server responded — reset circuit breaker
      consecutiveFailuresRef.current = 0;
      setServerDown(false);
      setData(json);
      ...
      if (!silent) setError(
        isAbort
          ? (language === 'ar' ? "انتهى وقت الطلب - الخادم بطيء" : "Request timed out - server is slow")
          : (err.message || "Failed to load monitor data")
      );
    } finally {
      if (!silent) setIsLoading(false);
    }
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8" dir={language === 'ar' ? 'rtl' : 'ltr'}>
        ...
        {/* ⚠️ Server-down banner */}
        {serverDown && (
          <div className="bg-rose-50 border border-rose-200 text-rose-700 rounded-[24px] p-5 font-bold flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <ShieldAlert className="w-5 h-5 shrink-0" />
              <span>
                {language === 'ar'
                  ? '❗ خادم المنصة لا يستجيب حالياً. التحديث التلقائي موقوف. يرجى الانتظار أو التواصل مع الدعم.'
                  : '\u26a0 Server is not responding. Auto-refresh paused. Please wait or contact support.'}
              </span>
            </div>
            <button
              onClick={() => {
              ...
              className="shrink-0 px-4 py-2 rounded-xl bg-rose-600 text-white text-sm font-black hover:bg-rose-700 transition-all"
            >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <SectionCard title={language === 'ar' ? "المسارات السريعة" : "Quick Links"} icon={Bell}>
            ...
            <Link href="/super-admin/schools" className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3 font-bold hover:bg-slate-100 transition-all">
